...
prompt = ChatPromptTemplate.from_template(
    """
    Is the Statement below accurate or reasonable given the Wikipedia content provided?

    If no Wikipedia content was provided, simply indicate that you weren't able to find
    anything related to the statement on Wikipedia.
...
async def check_wikipedia(statement: str) -> WikipediaCheckOutput:
    return await chain.ainvoke(statement)


async def acheck_wikipedia(statements: List[str]) -> List[WikipediaCheckOutput]:
    all_checks = []
